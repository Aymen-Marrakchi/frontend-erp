...
export default function OnlineSalesEmployees() {
  ...
  const handleCreate = async () => {
    ...
    try {
      setSubmitting(true);
      setFormError("");
      const result = await salesService.createEmployee({
        ...form,
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `sales-employees-${new Date().toISOString().split("T")[0]}.csv`;
    a.click();
    ...
    <ProtectedRoute allowedRoles={["ADMIN"]}>
      <div className="space-y-6">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            <p className="mb-2 text-[11px] font-medium uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">
              {t("onlineSalesSubtitle")}
            </p>
            <h1 className="text-3xl font-bold tracking-tight text-slate-950 dark:text-white">
              {t("onlineSales")}{" "}
              <span className="text-slate-400 dark:text-slate-500">{t("employees")}</span>
            </h1>
          </div>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800"
            >
              <Download size={15} />
              {t("export")}
            </button>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          {[
            {
              label: t("totalEmployeesKpiShort"),
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-6 py-5 dark:border-slate-800">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                {filtered.length} {t("ofText")} {employees.length} {t("records")}
              </p>
            </div>
