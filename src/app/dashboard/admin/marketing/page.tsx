...
export default function AdminMarketingPage() {
  ...
  const fetchAll = async () => {
    try {
      ...
      const [emps, st] = await Promise.all([
        marketingService.getAllEmployees(),
  ...
  const handleCreate = async () => {
    ...
    try {
      setSubmitting(true);
      setFormError("");
      const result = await marketingService.createEmployee({
        ...form,
      ...
      <div className="space-y-6">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 transition hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed dark:border-slate-800 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800"
            >
              <Download size={15} />
              {t("export")}
            </button>
        ...
        <div className={`${surface} overflow-hidden`}>
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-6 py-5 dark:border-slate-800">
            <div>
              <h2 className="text-lg font-semibold text-slate-950 dark:text-white">Marketing Team</h2>
